#!/usr/bin/env python3
"""把 fonts/ 下的字体文件以 base64 写入 js/fonts.js 的 FONT_B64 常量。

替换 js/fonts.js 后字体内嵌数据与 fonts/ 保持一致；内容未变时重复执行无副作用。
"""
import base64
import pathlib
import re
...
def main():
    ...
    # 显式写 LF：Windows 下默认会转成 CRLF，导致内容未变时文件字节也在变
